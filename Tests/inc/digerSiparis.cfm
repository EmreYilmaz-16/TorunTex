...
,ISNULL((SELECT SUM(STOCK_IN-STOCK_OUT) AS S_IN FROM #dsn2#.STOCKS_ROW  WHERE STOCK_ID =#attributes.STOCK_ID# and STORE=o.DELIVER_DEPT_ID and STORE_LOCATION=O.LOCATION_ID ),0) AS URETIM
...
WHERE STOCK_ID =#attributes.STOCK_ID# AND ORDER_STAGE=259 AND ORDER_STATUS=1 AND ORR.QUANTITY>ISNULL((SELECT SUM(STOCK_IN-STOCK_OUT) AS S_IN FROM #dsn2#.STOCKS_ROW  WHERE STOCK_ID =#attributes.STOCK_ID# and STORE=o.DELIVER_DEPT_ID and STORE_LOCATION=O.LOCATION_ID ),0)
</cfquery>
