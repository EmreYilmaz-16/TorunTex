<cfquery name="GETSKU" datasource="#dsn#">
    SELECT * FROM #dsn#.STOCKS WHERE PRODUCT_CODE_2 = '#attributes.SKU_CODE#'
</cfquery>

    <CFIF GETSKU.RECORDCOUNT EQ 0>
<cfquery name="getStocks" datasource="#dsn2#">
    
SELECT T.*,S.PRODUCT_NAME FROM (
SELECT STOCK_ID,LOT_NO,SUM(STOCK_IN-STOCK_OUT) BAKIYE FROM w3Toruntex_2024_1.STOCKS_ROW AS SR 
WHERE SR.STORE=#attributes.FromLocationId# AND SR.STORE_LOCATION=#attributes.ToLocationId#
GROUP BY STOCK_ID,LOT_NO
) AS T
LEFT JOIN w3Toruntex_1.STOCKS AS S ON S.STOCK_ID=T.STOCK_ID
WHERE BAKIYE>0 
AND S.STOCK_ID=#GETSKU.STOCK_ID#
ORDER BY STOCK_ID
</cfquery>


<cfelse>
<CFSET getStocks.recordCount=0>
    
</CFIF>