...
SELECT 
    ...
    ,CASE WHEN SR.PROCESS_TYPE=71 THEN 3 WHEN SR.PROCESS_TYPE=110 THEN 0 ELSE 2 END AS SIRALAMA
FROM #dsn2#.STOCKS_ROW AS SR
LEFT JOIN #dsn#.STOCKS_LOCATION AS SL ON SL.LOCATION_ID = SR.STORE_LOCATION
...
LEFT JOIN ( 
SELECT PROCESS_CAT PI1,COMPANY_ID,FIS_TYPE AS PROCESS_TYPE,0 AS PS,FIS_ID FROM	#dsn2#.STOCK_FIS 
UNION ALL 
SELECT PROCESS_CAT PI1,COMPANY_ID,SHIP_TYPE AS PROCESS_TYPE ,1 AS PS,SHIP_ID AS FIS_ID FROM	#dsn2#.SHIP 
) AS SF
ON SF.FIS_ID = S1.UPD_ID AND SF.PS=S1.TS
LEFT JOIN #dsn3#.SETUP_PROCESS_CAT AS PC ON PC.PROCESS_TYPE = SF.PROCESS_TYPE
	AND PC.PROCESS_CAT_ID = SF.PI1
LEFT JOIN #dsn#.COMPANY AS C ON C.COMPANY_ID= ISNULL(SF.COMPANY_ID,S1.COMPANY_ID)
WHERE S1.LOT_NO='#LOT_NO#'
 ORDER BY PROCESS_DATE DESC,SIRALAMA,UPD_ID,S1.STOCK_OUT DESC
        </cfquery>
