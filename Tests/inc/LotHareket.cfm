...
LEFT JOIN ( 
SELECT PROCESS_CAT,FIS_TYPE AS PROCESS_TYPE,0 AS PS,FIS_ID FROM	w3Toruntex_2023_1.STOCK_FIS 
UNION ALL 
SELECT PROCESS_CAT,SHIP_TYPE AS PROCESS_TYPE ,1 AS PS,SHIP_ID AS FIS_ID FROM	w3Toruntex_2023_1.SHIP 
) AS SF
ON SF.FIS_ID = S1.UPD_ID AND SF.PS=S1.TS
LEFT JOIN w3Toruntex_1.SETUP_PROCESS_CAT AS PC ON PC.PROCESS_TYPE = SF.PROCESS_TYPE
	AND PC.PROCESS_CAT_ID = SF.PROCESS_CAT
WHERE S1.LOT_NO='#LOT_NO#'
 ORDER BY UPD_ID,SR.STOCK_OUT DESC
        </cfquery>
