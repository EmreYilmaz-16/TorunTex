...
ORDER BY SR.PROCESS_DATE DESC
 ...
 SELECT TOP 5 SFR.AMOUNT,C.NICKNAME ,SL.COMMENT,O.ORDER_NUMBER,S.PRODUCT_NAME FROM #dsn#_2023_1.STOCK_FIS_ROW AS SFR
 LEFT JOIN #dsn#_2023_1.STOCK_FIS AS SF ON SF.FIS_ID=SFR.FIS_ID
 LEFT JOIN #dsn#_2023_1.STOCKS_ROW AS SR ON SR.PBS_RELATION_ID=SFR.PBS_RELATION_ID AND SFR.FIS_ID =SR.UPD_ID
 LEFT JOIN #dsn3#.ORDER_ROW AS ORR ON ORR.WRK_ROW_ID=SR.PBS_RELATION_ID
 LEFT JOIN #dsn3#.ORDERS AS O ON O.ORDER_ID=ORR.ORDER_ID
 LEFT JOIN #dsn#.COMPANY AS C ON C.COMPANY_ID =O.COMPANY_ID
 LEFT JOIN #dsn#.STOCKS_LOCATION AS SL ON SL.DEPARTMENT_ID=SR.STORE AND SL.LOCATION_ID=SR.STORE_LOCATION
 LEFT JOIN #dsn3#.STOCKS AS S ON S.STOCK_ID=SR.STOCK_ID
