<cfquery name="getld" datasource="#dsn3#">
    ...
    select WRK_ROW_ID,T.FIS_NUMBER,LNO,STORE,STORE_LOCATION,STOCK_IN,SS.PRODUCT_CODE,SS.PRODUCT_NAME,SS.PRODUCT_CODE_2,SS.PRODUCT_NAME+'|'+LNO+'||'+CONVERT(VARCHAR,STOCK_IN) AS NEW_LOT_BARCODE,SL.COMMENT,DATEADD(HOUR,3,T.RECORD_DATE) RECORD_DATE,E.EMPLOYEE_NAME+' '+E.EMPLOYEE_SURNAME AS REMP
 from w3Toruntex_1.ORDER_ROW  AS ORR
 OUTER APPLY(
  SELECT FIS_NUMBER,LOT_NO AS LNO,STORE,STORE_LOCATION,STOCK_IN,sf.RECORD_EMP,sf.RECORD_DATE FROM #dsn2#.STOCKS_ROW AS SR
LEFT JOIN #dsn2#.STOCK_FIS AS SF ON SF.FIS_ID=SR.UPD_ID
