<cfif isDefined("attributes.tasima")>
   ...
   <cfquery name="getLOTS" datasource="#dsn2#">
        SELECT SUM(STOCK_IN-STOCK_OUT),LOT_NO,STOCK_ID,PRODUCT_ID FROM w3Toruntex_2024_1.STOCKS_ROW WHERE STORE=18 AND STORE_LOCATION=#listGetAt(attributes.LOC_OUT,2,"-")# AND SHELF_NUMBER IS NOT NULL GROUP BY LOT_NO,STOCK_ID,PRODUCT_ID HAVING SUM(STOCK_IN-STOCK_OUT)>0
    </cfquery>
    <cfdump var="#getLOTS#">
