...
SELECT * FROM (SELECT C.NICKNAME
	...
	,ISNULL((
        SELECT 
SUM( CASE WHEN STOCK_OUT IS NULL THEN -1 ELSE 1 END )
FROM #DSN2#.STOCKS_ROW WHERE STORE=O.DELIVER_DEPT_ID
AND PBS_RELATION_ID=ORR.WRK_ROW_ID
AND STORE_LOCATION=O.LOCATION_ID GROUP BY STOCK_ID,PBS_RELATION_ID
...
<cfif session.ep.userid eq 1411114>
    <cfdump var="#getOrder#">
</cfif>
