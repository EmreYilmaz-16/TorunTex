...
WHERE ORDER_ROW.ORDER_ROW_ID = #arguments.ORDER_ROW_ID#    
   ...
   <cfquery name="GETDATA2" datasource="#DSN3#">
    ...
    ISNULL((
        SELECT sum(STOCK_IN-STOCK_OUT) STOCK_IN FROM #dsn2#.STOCKS_ROW where PBS_RELATION_ID=ORDER_ROW.WRK_ROW_ID and STORE=#GETDATA.DELIVER_DEPT_ID# and STORE_LOCATION=#GETDATA.LOCATION_ID#
    ),0) AS R_AMOUNT,
...
<cfquery name="GETD" datasource="#DSN2#">
    SELECT SFR.LOT_NO,SFR.AMOUNT,S.PRODUCT_NAME,S.PRODUCT_DETAIL,S.PRODUCT_CODE_2,SL.COMMENT,O.ORDER_NUMBER FROM #dsn2#.STOCK_FIS AS SF 
