...
SELECT sum(STOCK_IN) STOCK_IN FROM w3Toruntex_2023_1.STOCKS_ROW where PBS_RELATION_ID=ORDER_ROW.WRK_ROW_ID and STORE=7 and STORE_LOCATION=5),0)/QUANTITY) AS TAMAMLANMA
   ...
   <cfquery name="GETDATA2" datasource="#DSN3#">
    ...
    ISNULL((
        SELECT sum(STOCK_IN) STOCK_IN FROM w3Toruntex_2023_1.STOCKS_ROW where PBS_RELATION_ID=ORDER_ROW.WRK_ROW_ID and STORE=7 and STORE_LOCATION=5
    ),0) AS R_AMOUNT,
...
<cfloop list="#attributes.SARF_STOCK_ID_LIST#" index="i" item="it">
    <cfoutput>
        ...
        <cfset "attributes.lot_no#i#"=arguments.LOT_NO>
