...
<cffunction name="getOrders" access="remote" httpMethod="Post" returntype="any" returnFormat="json">
        ...
        <cfquery name="GETDATA" datasource="#DSN3#">
            ...
            WHERE ORR.PRODUCT_ID=#arguments.PRODUCT_ID# AND ORR.UNIT2 LIKE '%#attributes.STATION#%'
        </cfquery>
...
SELECT sum(STOCK_IN) STOCK_IN FROM w3Toruntex_2023_1.STOCKS_ROW where PBS_RELATION_ID=ORDER_ROW.WRK_ROW_ID and STORE=7 and STORE_LOCATION>20),0)/QUANTITY) AS TAMAMLANMA
   ...
   <cfquery name="GETDATA2" datasource="#DSN3#">
    ...
    ISNULL((
        SELECT sum(STOCK_IN) STOCK_IN FROM w3Toruntex_2023_1.STOCKS_ROW where PBS_RELATION_ID=ORDER_ROW.WRK_ROW_ID and STORE=7 and STORE_LOCATION>20
    ),0) AS R_AMOUNT,
