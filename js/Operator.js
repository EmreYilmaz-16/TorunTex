...
function getProducts(STATION) {
  ...
  qstr +=
    " SELECT PRODUCT_NAME,STOCKS.PRODUCT_ID,STOCK_ID,PRODUCT_DETAIL FROM "+dsn3+".STOCKS LEFT JOIN "+dsn3+".PRODUCT_INFO_PLUS ON PRODUCT_INFO_PLUS.PRODUCT_ID=STOCKS.PRODUCT_ID ";
  qstr +=
    " WHERE PRODUCT_CATID NOT IN (26) AND PROPERTY1 LIKE '%" +
    STATION +
    "%'  UNION ALL   SELECT S.PRODUCT_NAME,S.PRODUCT_ID,S.STOCK_ID,S.PRODUCT_DETAIL FROM "+dsn3+".ORDER_ROW ";
  qstr +=
    " INNER JOIN "+dsn3+".STOCKS AS S ON S.STOCK_ID =ORDER_ROW.STOCK_ID  WHERE UNIT2 LIKE '%" +
    STATION +
...
function YenidenFisYazdir() {
  ...
  str +=
    " LEFT JOIN "+dsn3+".ORDER_ROW AS ORR ON ORR.WRK_ROW_ID=SR.PBS_RELATION_ID LEFT JOIN "+dsn3+".ORDERS AS O ON O.ORDER_ID=ORR.ORDER_ID";
  str +=
    " INNER JOIN "+dsn+".STOCKS_LOCATION AS SL ON SL.DEPARTMENT_ID=SF.DEPARTMENT_IN AND SL.LOCATION_ID=SF.LOCATION_IN";
  str += " INNER JOIN "+dsn3+".STOCKS AS S ON S.STOCK_ID=SR.STOCK_ID";
