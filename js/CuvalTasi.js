...
function SearchBarcode(el, ev) {
  if (ev.keyCode == 13) {
    ...
    var Qstr1="SELECT SUM(STOCK_IN-STOCK_OUT),STORE DELIVER_DEPT_ID,STORE_LOCATION LOCATION_ID,STOCK_ID,WRK_ROW_ID FROM "+dsn2+".STOCKS_ROW WHERE  LOT_NO='"+LotNo+"' GROUP BY STORE,STORE_LOCATION ,STOCK_ID,WRK_ROW_ID HAVING SUM(STOCK_IN-STOCK_OUT)>0"
